...
def retrieve_recipes(ingredient_list: list[str]):
    ...
    recipes = Recipe.objects.exclude(ingredient1_embedding__isnull=True).exclude(
        ingredient1_embedding=[]
    )
...
def gemini_inference(request):
    if request.method == "POST" and request.FILES.get("food_photo"):
        ...
        try:
            response = client.models.generate_content(
                ...
                contents=[
                    ...
                    types.Part.from_bytes(
                        data=image_data, mime_type=food_photo.content_type
                    ),
                ],
            )
            ...
            response = client.models.generate_content(
                model="gemini-3-flash-preview", contents=[formatted_recipe_prompt]
            )
